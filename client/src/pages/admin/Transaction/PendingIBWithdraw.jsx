...
function PendingIBWithdraw() {
  ...
  const pendingData = [
    { id: 1, ibName: 'John Doe IB', email: 'john@example.com', amount: 200, method: 'Bank Transfer', date: '2025-11-10' },
  ]

  const filteredData = pendingData.filter(item => 
    item.ibName.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 lg:ml-64 mt-12 p-4 lg:p-8 w-full bg-gray-50 min-h-screen">
        <h1 className="text-lg font-bold mb-4">Pending IB Withdraw</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-y border-black">
                  ...
                  <th className="text-left py-0.5 px-2 font-medium text-gray-600 text-xs">IB Name</th>
              ...
              <tbody>
                {filteredData.slice(0, entries).map((item, index) => (
                  <tr key={item.id} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                    ...
                    <td className="py-0.5 px-2 text-xs">{item.ibName}</td>
